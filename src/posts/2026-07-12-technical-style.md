---
layout: post
title: "現代 Web 應用架構設計：高併發 API 伺服器與快取策略實戰"
...
<div class="font-sans leading-relaxed text-gray-800">
...
<div class="my-8 rounded-lg overflow-hidden border border-gray-700 bg-[#1e293b] p-4 shadow-xl">
  <img src="/travel/img/posts/tech_architecture.jpg" alt="Web應用服務架構圖" class="w-full h-auto rounded border border-gray-800">
  <div class="mt-3 text-center text-xs text-gray-400 font-mono">
    Figure 1.0: Full-Stack Decoupled Architecture with Load-Balanced API Server and Redis Caching
  </div>
</div>
...
<div class="overflow-x-auto my-6">
  <table class="min-w-full divide-y divide-gray-200 border border-gray-100 text-sm">
    <thead class="bg-gray-50 font-bold">
      <tr>
        <th class="px-4 py-3 text-left">快取更新模式 (Pattern)</th>
        <th class="px-4 py-3 text-left">讀操作流程</th>
        <th class="px-4 py-3 text-left">寫操作流程</th>
        <th class="px-4 py-3 text-left">優點 / 缺點</th>
      </tr>
    </thead>
    <tbody class="divide-y divide-gray-200 font-mono text-xs text-gray-700">
      <tr>
        <td class="px-4 py-3 font-sans font-semibold text-gray-900">Cache-Aside (旁路快取)</td>
        <td class="px-4 py-3 font-sans">先查快取，若 Miss 則查 DB 並回寫快取。</td>
        <td class="px-4 py-3 font-sans">先更新 DB，再直接刪除舊快取。</td>
        <td class="px-4 py-3 font-sans text-green-700">最常用，簡單安全；可能有短暫不一致。</td>
      </tr>
      <tr class="bg-gray-50/50">
        <td class="px-4 py-3 font-sans font-semibold text-gray-900">Write-Through (直寫快取)</td>
        <td class="px-4 py-3 font-sans">同上，快取由代理層透明處理。</td>
        <td class="px-4 py-3 font-sans">寫入時，由快取層同步寫入 DB 後返回。</td>
        <td class="px-4 py-3 font-sans text-gray-600">無不一致問題；寫延遲較高，寫壓力大。</td>
      </tr>
      <tr>
        <td class="px-4 py-3 font-sans font-semibold text-gray-900">Write-Behind (後寫快取)</td>
        <td class="px-4 py-3 font-sans">同上。</td>
        <td class="px-4 py-3 font-sans">唯獨寫入快取，異步批量將更新刷回 DB。</td>
        <td class="px-4 py-3 font-sans text-red-700">極致的寫效能；DB 宕機恐丟失未落盤資料。</td>
      </tr>
    </tbody>
  </table>
</div>
