...
### 2026-07-12 — 新增設計風格分析文件 `doc/style.md`

由資深 UI/UX 設計師視角，將全站現行的視覺與互動風格系統化整理為獨立文件
[`doc/style.md`](./style.md)，供後續設計／開發時對齊風格：
*   **文件結構分兩區塊**：A. 抽象設計準則（10 條可延續的設計心法，如「中性灰主導、
    暖褐點綴」「Token-First」「層次靠排版而非顏色」「尊重 CJK 排版慣例」「反過度工程」等）；
    B. 具體網頁設計細節（色彩 Token 對照表、字型系統、佈局、Navbar/Hero/Footer/標籤/
    按鈕/分頁器/TOC/`.prose` 等元件規格）。
*   **記錄使用者提供的四色色票**（`#222831`/`#393E46`/`#948979`/`#DFD0B8`）及其「冷深灰
    家族＝文字、暖褐米家族＝強調」的分工原則。
