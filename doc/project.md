...
- Jekyll + `jekyll-feed` / `jekyll-sitemap` (已於 2026-07-11 移除 `jekyll-paginate` 改用 JS 前端分頁)
...
posts/index.html     文章目錄列表表格 (橫線表格，前端分頁 100 筆/頁)
about.html           靜態網頁
...
- **雙頁面通用前端分頁機制**：因 Jekyll 內建的 `jekyll-paginate` 僅能全站單一頁面套用相同分頁大小，為同時支援首頁（5 筆/頁）與文章目錄（100 筆/頁）的不同需求，決定改用前端 JS 控制分頁。這能在保留全站 HTML 的 SEO 好處下，同時實現流暢的淡入淡出 Loading 轉場、滾動置頂、及 URL 參數同步，且移除了對 `jekyll-paginate` 套件的依賴。
